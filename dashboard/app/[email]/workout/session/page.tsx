...
export default function WorkoutSessionPage() {
  ...
  // Mock workout dates
  const workoutDates = [
    { date: "Jul 1, 25", time: "08:00 AM - 09:30 AM", note: "Shoulder press felt strong today, increased weight", warmupCompleted: true },
    { date: "Jun 24, 25", time: "08:00 AM - 09:30 AM", note: "Recovery was good since last session", warmupCompleted: false },
    { date: "Jun 17, 25", time: "08:00 AM - 09:30 AM", note: "Great leg day, focused on hip thrusts and squat form", warmupCompleted: true },
  ];
  ...
  const exercisesData: ExerciseHistory[] = [
    ...
    },
    {
      ...
      history: [
        [
          ...
          { sets: ["77×8", "69×10", "67×11", "65×9", "63×8"], completed: false },
    ...
    },
    {
      ...
      history: [
        [
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true },
          { sets: ["59×10", "57×10", "55×8"], completed: true },
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true }
        ]
      ]
    },
    {
      ...
      history: [
        [
          { sets: ["77×10", "75×11", "73×9", "71×10", "69×11"], completed: false },
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true },
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true }
        ]
      ]
    },
    {
      ...
      history: [
        [
          { sets: ["68×10", "65×11", "64×8", "62×9", "60×8"], completed: false },
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true },
          { sets: ["74×9", "72×10", "70×10", "72×10", "70×10"], completed: true }
        ]
      ]
  ...
  const getExerciseTrend = (exercise: ExerciseHistory, dateIndex: number): 'up' | 'down' | 'neutral' => {
    ...
    if (dateIndex === 1) {
      if (['Cable Overhead Triceps Extension', 'One-leg Leg Extension', 'Seated DB Lateral Raise'].includes(exercise.name)) {
        return 'down';
      } else if (['Heels Elevated Zercher Squat', 'Scrape Rack L-Seated Shoulder Press'].includes(exercise.name)) {
        return 'up';
      }
    } else if (dateIndex === 2) {
      if (['Seated DB Lateral Raise'].includes(exercise.name)) {
        return 'down';
      } else if (['Cable Overhead Triceps Extension', 'One-leg Leg Extension', 'Scrape Rack L-Seated Shoulder Press'].includes(exercise.name)) {
        return 'up';
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 mt-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            <button className="px-4 py-1 rounded bg-[#DD3333] text-white text-sm">View</button>
            <button className="px-4 py-1 rounded bg-[#142437] text-white text-sm">Edit</button>
          </div>
        </div>
      </div>
            ...
            <thead>
              <tr className="text-left">
                ...
                {workoutDates.map((date, index) => (
                  <th key={index} className="p-3 border-b border-[#22364F] min-w-[200px]">
                    <div className="font-semibold">{date.date}</div>
                    <div className="text-xs text-gray-400">Time: {date.time}</div>
                    <div className="text-xs text-gray-400">Note: {date.note}</div>
              ...
              {exercisesData.map((exercise, exIndex) => (
                <tr key={exIndex} className="border-b border-[#22364F]">
                  <td className="p-3">
                    ...
                    <div className="text-xs text-gray-400">{exercise.standardWeight} × {exercise.standardReps} × {exercise.standardSets} sets{exercise.perSide ? ' (per leg)' : ''}</div>
                  </td>
                    ...
                    return (
                      <td 
                        ...
                        className={`p-2 relative ${
                          !setData.completed ? 'border-2 border-red-500' : 
                          trend === 'up' ? 'border-2 border-green-500' : 
                          trend === 'down' ? 'border-2 border-red-500' : ''
                        }`}
                      >
                        <div className="flex flex-col w-full">
                          {setData.sets.map((set, setIndex) => (
                            <div 
                              ...
                              style={{
                                borderRadius: setIndex === 0 ? '0.375rem 0.375rem 0 0' : 
                                            setIndex === setData.sets.length - 1 ? '0 0 0.375rem 0.375rem' : 'none'
                              }}
                            >
                              {set}
