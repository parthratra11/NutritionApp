...
const IntroScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
...
const progressWidth = `${(SCREEN_INDEX / TOTAL_SCREENS) * 100}%`;
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: isDarkMode ? '#111827' : '#1A2330' }}>
      {/* Progress Bar */}
      <View style={{ height: 6, backgroundColor: '#e5e7eb', marginHorizontal: 20, borderRadius: 3, marginTop: 32 }}>
        <View style={{ height: '100%', backgroundColor: '#E11D48', borderRadius: 3, width: progressWidth }} />
      </View>
      <View style={introStyles.content}>
        ...
        >
          <View style={{ paddingVertical: 32, paddingHorizontal: 18, width: '100%', alignItems: 'center' }}>
            <Text style={[introStyles.text, { marginBottom: 12, fontSize: 17, color: '#232946', backgroundColor: 'transparent', textAlign: 'center', marginRight: 29 }]}>
              Before we jump into building your dream program, we just need a few answers from you—nothing too wild, promise!
            </Text>
            <Text style={[introStyles.subtext, { fontSize: 14, color: '#232946', backgroundColor: 'transparent', textAlign: 'center', marginRight: 30 }]}>
              Think of this like giving your future self a high-five. Your honest responses help us tailor everything just for you—from training to nutrition and everything in between!
            </Text>
            <TouchableOpacity
...
const MeasurementSystemScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: isDarkMode ? '#111827' : '#1A2330' }}>
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        ...
        >
          ...
          <View style={{ zIndex: 2, width: '100%', alignItems: 'center',backgroundColor: 'transparent' }}>
            <Text style={{ color: '#fff', fontSize: 22, fontWeight: 'bold', textAlign: 'center', marginBottom: 48, marginTop: 10 }}>
              Preferred Measurement{'\n'}System
            </Text>
            <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 24 ,backgroundColor: 'transparent'}}>
              <TouchableOpacity
                style={{
                  borderWidth: 1,
                  borderColor: '#fff',
...
const HeightWeightScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
  const { formData, setFormData } = useFormContext();
...
const BodyFatScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
  const { formData, setFormData } = useFormContext();
      ...
      <View style={heightWeightStyles.content}>
        <View style={heightWeightStyles.card}>
          ...
          <View style={[heightWeightStyles.inputContainer, { justifyContent: 'center' }]}>
            ...
            <View style={[heightWeightStyles.inputWrapper, { marginTop: 18, position: 'relative' }]}>
              <TextInput
                style={[heightWeightStyles.input, { paddingRight: 40 }]}
...
const ExperienceScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
  const { formData, setFormData } = useFormContext();
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 12 }}>
            <TouchableOpacity
              style={{
                ...
                marginRight: 18,
                backgroundColor: formData.strengthTrainingExperience === 'Yes' ? '#fff' : 'transparent',
              }}
              onPress={() => {
            ...
            <TouchableOpacity
              style={{
                ...
                paddingHorizontal: 38,
                marginLeft: 18,
                backgroundColor: formData.strengthTrainingExperience === 'No' ? '#fff' : 'transparent',
              }}
              onPress={() => {
...
const StrengthCongratsScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
...
const StrengthLevelScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { isDarkMode } = useTheme();
  const { formData, setFormData } = useFormContext();
  ...
  const goNext = () => navigation.navigate('StrengthCompetency', { fromCongrats: true });
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      ...
      <View style={{ flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ width: '100%', position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, zIndex: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', pointerEvents: 'box-none' }}>
          <TouchableOpacity
            onPress={goBack}
            style={{
              padding: 10,
        ...
        >
          {exercises.map((exercise, idx) => (
            ...
            >
              <Text style={{ color: '#fff', fontSize: 18, fontWeight: 'bold', textAlign: 'center', marginBottom: 18 }}>
                {exercise.label}
              </Text>
              {exercise.description && (
                <Text style={{ color: '#fff', fontSize: 13, textAlign: 'center', marginBottom: 8, paddingHorizontal: 8 }}>
                  {exercise.description}
                </Text>
              )}
              {/* Weight Input */}
...
const StrengthCompetencyScreen: React.FC<{ navigation: NavigationProp<RootStackParamList> }> = ({ navigation }) => {
  const { formData, setFormData } = useFormContext();
  ...
  return (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 12 }}>
        {/* Arrows */}
        <View style={{ width: '100%', position: 'absolute', top: '-25%', bottom: 0, left: 0, right: 0, zIndex: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', pointerEvents: 'box-none' }}>
          <TouchableOpacity
            onPress={goBack}
            style={{
              padding: 10,
        ...
        {/* Comments */}
        <Text style={{ color: '#fff', fontSize: 15, marginTop: 30, marginBottom: 8, textAlign: 'center' }}>
...
const DreamBigScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation,route }) => {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        <Text style={{
          ...
          Whether it’s six-pack abs, lifting Thor’s hammer, or just feeling awesome, let us know what you’re chasing.
        </Text>
        <Text style={{
          color: '#fff',
          fontSize: 25,
...
const TopGoalsScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
...
const ObstacleExerciseScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
...
const DedicationLevelScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
...
const WeeklyFrequencyScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
  ...
  >
    <Ionicons name="chevron-forward" size={40} color="#fff" />
        ...
        <Text style={{
          ...
          How often in a week would you be prepared
          to train for maximal results?{'\n'}
        ...
        <View style={{
          ...
          <Text style={{ color: '#232946', fontSize: 28, fontWeight: 'bold', minWidth: 32, textAlign: 'center' }}>
            {frequency}
          </Text>
          <TouchableOpacity
            onPress={increment}
...
const OccupationScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,oute:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
...
const DietMedicalScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
  const [specialDiet, setSpecialDiet] = React.useState(formData.specialDiet ?? '');
  const [medicalConditions, setMedicalConditions] = React.useState(formData.medicalConditions ?? '');
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        <View style={{
          ...
          <Text style={{
            ...
            Do you currently follow any special diet (e.g. ketogenic, vegan), or wish to follow any special diet as part of this coaching program?
          </Text>
          <TextInput
            style={{
              backgroundColor: '#081A2FCC',
...
const LifestyleIntroScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('TrainingTimePreference', { screenIndex: SCREEN_INDEX + 1 })}
          >
...
const TrainingTimePreferenceScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>,route:any }> = ({ navigation,route }) => {
  const { formData, setFormData } = useFormContext();
  const [trainingTimePreference, setTrainingTimePreference] = React.useState(formData.trainingTimePreference ?? '');
...
const StressLevelScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const ActivityLevelScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  ...
  const activityOptions = [
    ...
    },
    {
      ...
      label: 'Somewhat active (e.g. you walk your dog several times a day or you commute by bicycle/on foot), 7,500 – 9,999 steps/day',
    },
    {
      value: 'Active',
      label: 'Active (e.g. full-time PT, literally on your feet most of the day), 10,000 – 12,500 steps/day',
    },
    {
      value: 'Very active',
      label: 'Very active (e.g. involved in physical labour), over 12,500 steps/day with intensive movement',
    },
  ];
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 16 }}>
        ...
        >
          <Text style={{
            ...
            Which of the following options best{'\n'}describes your activity level (this does NOT include exercise)?*
          </Text>
...
const SleepQualityScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const CaffeineIntakeScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const MenstrualCycleScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const SupplementsScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const GeneticsScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  const [wristCircumference, setWristCircumference] = React.useState(formData.wristCircumference ?? '');
  const [ankleCircumference, setAnkleCircumference] = React.useState(formData.ankleCircumference ?? '');
...
const EquipmentIntroScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('SkinfoldCalipers', { screenIndex: SCREEN_INDEX + 1 })}
          >
            <Ionicons name="chevron-forward" size={70} color="#232946" />
          </TouchableOpacity>
        </ImageBackground>
...
const SkinfoldCalipersScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const MyoTapeScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const FitnessTechScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const CardioEquipmentScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  const [cardioEquipment, setCardioEquipment] = React.useState<string[]>(formData.cardioEquipment ?? []);
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        <View style={{
          ...
          {gridRows.map((row, rowIdx) => (
            <View key={rowIdx} style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 18 }}>
              {row.map((option, idx) => (
                <TouchableOpacity
                  key={option.key}
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 24,alignItems: 'center',marginLeft:25 }}>
          {options.map(opt => (
            <TouchableOpacity
              key={opt.value}
...
const EquipmentAccessScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  ...
  const [cableTowerAdjustable, setCableTowerAdjustable] = React.useState(formData.cableTowerAdjustable ?? '');
  // Save to formData on change
  React.useEffect(() => {
    setFormData(prev => ({
      ...prev,
  ...
  const goBack = () => navigation.goBack();
  const goNext = () => navigation.navigate('EquipmentDifference', { screenIndex: SCREEN_INDEX + 1 });
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      ...
      <ScrollView contentContainerStyle={{ alignItems: 'center', paddingBottom: 40 }}>
        ...
        <Text style={{
          ...
          Note that you don’t need access to all of the below equipment to have a good training program. I’m mainly asking for the more unconventional pieces of equipment here.
        </Text>
        {/* 3x3 grid */}
        <View style={{
          flexDirection: 'row',
...
const EquipmentDifferenceScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  const [equipmentDifference, setEquipmentDifference] = React.useState(formData.equipmentDifference ?? '');
  ...
  const goBack = () => navigation.goBack();
  const goNext = () => navigation.navigate('CurrentProgramIntro', { screenIndex: SCREEN_INDEX + 1 });
...
const CurrentProgramIntroScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
...
const TypicalDietScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
...
const CurrentProgramScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData } = useFormContext();
  ...
  const handleUpload = async () => {
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/plain', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'image/*'],
...
const StrikeAPoseScreen: React.FC<{ navigation: NavigationProp<RootStackParamList>, route: any }> = ({ navigation, route }) => {
  const { formData, setFormData, handleSubmit, hasSubmitted } = useFormContext();
  const [photos, setPhotos] = React.useState<{ uri: string; name: string }[]>(formData.photos ?? []);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      ...
      {/* Progress Bar */}
      <View style={{ height: 6, backgroundColor: '#e5e7eb', marginHorizontal: 20, borderRadius: 3, marginTop: 32 }}>
        <View style={{ height: '100%', backgroundColor: '#E11D48', borderRadius: 3, width: '100%' }} />
      </View>
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
        ...
        >
          <Text style={{
            ...
            Please upload at least four full-body pictures (wearing shorts that don’t cover your knees): front, both sides, and back.{"\n"}
  ...
  return (
    <FormContext.Provider value={{ formData, setFormData, hasSubmitted, handleSubmit }}>
      <Stack.Navigator
        ...
        {screens.filter(s => s.name !== 'MeasurementSystem').map((screen) => (
          <Stack.Screen
            key={screen.name}
